...
export default function Navbar() {
  ...
  return (
    <nav className={`fixed w-full z-50 transition-all duration-300 ${isScrolled ? 'bg-gray-950/95 backdrop-blur-sm' : 'bg-transparent'
      }`}>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16">
          ...
          <div className="hidden md:flex items-center space-x-8">
            <NavLink to="/movies" icon={<Film className="h-4 w-4" />} text="Movies" />
            <NavLink to="/tv-shows" icon={<Tv className="h-4 w-4" />} text="TV Shows" />
            <NavLink to="/categories" icon={<Grid className="h-4 w-4" />} text="Categories" />
            <button
